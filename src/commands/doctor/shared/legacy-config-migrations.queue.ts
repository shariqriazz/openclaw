...
const QUEUE_MODE_RULES: LegacyConfigRule[] = [
  {
    ...
    message:
      'messages.queue.mode uses a retired queue mode; use steer, redirect, followup, collect, or interrupt. Run "openclaw doctor --fix".',
    match: isRetiredQueueMode,
  },
  {
    path: ["messages", "queue", "byChannel"],
    message:
      'messages.queue.byChannel contains a retired queue mode; use steer, redirect, followup, collect, or interrupt. Run "openclaw doctor --fix".',
